...
template <typename T>
inline bool is_expression(const std::unique_ptr<Expression>& exp) {
    if (!exp) return false;
    return std::holds_alternative<T>(exp->expression);
}
...
inline NodeResult make(std::unique_ptr<Expression>&& expr) {
